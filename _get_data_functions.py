...
def stimulus(CATEGORY, STIMULUS_NAME):
    ...
    return cv2.imread(COLLECTION_PATH+'/Dataset/'+CATEGORY+'/STIMULI/'+STIMULUS_NAME, 1)

#########################################################################################

def fixation_map(CATEGORY,FIXATION_NAME):

    ''' This functions returns the matrix of pixels of the fixation map
        of a specified stimulus generated through eye-tracking experiment.
    ...
    #_, file_extension = os.path.splitext(os.listdir(COLLECTION_PATH + '/'+ DATASET_NAME + '/FIXATION_MAPS/')[0])

    # Get the matrix

    fixation_map =   fixation_map = cv2.imread( COLLECTION_PATH+'/Dataset/'+CATEGORY+'/FIXATION_MAPS/'+FIXATION_NAME, 0)
...
def orig_saliency_map(CATEGORY,ORIG_SALIENCY_NAME):

    ''' This functions returns the matrix of pixels of the saliency map
        of a specified stimulus generated through eye-tracking data. Saliency map has been obtained by convolving
    ...
    # Get the matrix
    saliency_map = cv2.imread(COLLECTION_PATH + '/Dataset/'+ CATEGORY+'/GND_SALIENCY_MAPS/'+ ORIG_SALIENCY_NAME, 0)
...
def saliency_map(MODEL_NAME, CATEGORY,SALIENCY_NAME):

    ''' This functions returns the matrix of pixels of the saliency map
        generated by a model. Saliency map has been obtained by convolving
        the fixation map with a proper gaussian filter (corresponding to
        one degree of visual angle).
    ...
    # Get the matrix
    saliency_map = cv2.imread(COLLECTION_PATH + '/Model_Results/'+ MODEL_NAME + '/'+CATEGORY+'/SALIENCY_MAPS/'+ SALIENCY_NAME, 0)
...
def scanpath(DATASET_NAME, CATEGORY, STIMULUS_NAME,subject = 0):
    ...
    if not subject:
        list_of_subjects = os.listdir(COLLECTION_PATH+'/'
                            +DATASET_NAME+'/'+CATEGORY+'/SCANPATHS/'
                            +file_name)
